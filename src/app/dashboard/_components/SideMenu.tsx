...
const SideMenu = ({ isOpen, setIsOpen }: SideMenuProps) => {
  ...
  const menuSelectedClasses =
    "flex items-center gap-3 px-4 py-3 bg-brown-dark text-primary-gold-light rounded-sm transition-colors group"
  const menuClasses =
    "flex items-center gap-3 px-4 py-3 text-stone-600 hover:text-primary-gold rounded-sm transition-colors group"
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed inset-y-0 left-0 z-50 w-72 bg-page border-r border-stone-200
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
          ...
          <Link
            ...
            <Archive className="transition-colors" />
          ...
          <a
            className="flex items-center gap-3 px-4 py-3 text-stone-600 hover:text-primary-gold rounded-sm transition-colors group"
            href="https://wa.me/34691250100?text=Necesito%20ayuda%20con%20mi%20portal%20de%20proveedor"
            target="_blank">
            <Headset className="transition-colors" />
            <span className="font-medium text-sm">Support</span>
          </a>
          <Link
            className={pathname === "/dashboard/quality-commitment" ? menuSelectedClasses : menuClasses}
            href="/dashboard/quality-commitment">
            <ShieldCheck className="transition-colors" />
            <span className="font-medium text-sm">Quality Commitment</span>
          </Link>
        </nav>
      </aside>
    </>
