...
export const chatWithConcierge = async (history: { role: "user" | "model"; text: string }[], message: string) => {
  try {
    const chat = ai.chats.create({
      ...
      config: {
        systemInstruction:
          "You are the Marhaba Marbella Luxury Concierge. You assist elite clients in finding yachts, villas, and experiences in Marbella. You are sophisticated, knowledgeable, and always prioritize privacy and excellence. Keep responses concise and helpful.",
        tools: [{ googleSearch: {} }]
      }
    })
    ...
    return {
      text:
        response.text ||
        "I'm sorry, I couldn't process that. How can I assist you further with Marbella's luxury offerings?",
