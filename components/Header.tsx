...
const Header = () => {
  ...
  const categoryGroups = [
    {
      label: 'Living',
      categories: ['Real Estate', 'Property Management', 'Villa Rental']
    },
    {
      label: 'Mobility',
      categories: ['Cars & Chauffeurs', 'Yachts', 'Private Jets & Transfers']
    },
    {
      label: 'Lifestyle',
      categories: ['Dining & Nightlife', 'Golf Experiences']
    },
    {
      label: 'Services',
      categories: ['Domestic Staff', 'Medical Care']
    }
  ]
  ...
  return (
    <header className={`fixed top-0 left-0 z-50 w-full transition-all duration-300 h-24 flex items-center ${
      (scrolled || !isHome) ? 'bg-page/95 backdrop-blur-md border-b border-brand-border shadow-sm' : 'bg-transparent'
    }`}>
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-6 lg:px-8">
        <div className="flex items-center gap-4 cursor-pointer" onClick={() => navigate('/')}>
          <div className={`flex size-10 items-center justify-center ${scrolled || !isHome ? 'text-deep-brown' : 'text-white'}`}>
          ...
          <h2 className={`font-display text-lg font-bold tracking-widest drop-shadow-md ${scrolled || !isHome ? 'text-deep-brown' : 'text-white'}`}>MARHABA</h2>
        </div>

         <nav className="hidden lg:flex gap-10">
          {categoryGroups.map((group) => (
            <div key={group.label} className="relative group h-24 flex items-center">
              <button className={`text-[11px] uppercase tracking-[0.2em] font-semibold flex items-center gap-1.5 transition-colors duration-300 ${
        ...
        <div className="flex items-center gap-6">
          <div className="hidden md:flex items-center gap-4">
            <button className={`${scrolled || !isHome ? 'text-brand-primary' : 'text-white'} hover:opacity-80 transition-colors`}>
              <Search className="w-5 h-5" />
            </button>
            <div className={`h-4 border-l ${scrolled || !isHome ? 'border-brand-border' : 'border-[#E7E5E4]'} opacity-50`}></div>
            <SignedOut>
              <SignInButton>
                <button
                  className={`text-sm font-medium transition-colors drop-shadow-md ${
                    scrolled || !isHome ? 'text-brand-primary hover:text-primary-gold' : 'text-white hover:text-primary-gold'
                  }`}
                >
                  Log In
                </button>
          ...
          <button 
            ...
            className={`h-11 items-center justify-center rounded-sm px-8 text-[11px] uppercase tracking-[0.2em] font-bold transition-all active:scale-95 shadow-lg flex ${
              scrolled || !isHome ? 'bg-brand-primary text-white hover:bg-primary-gold' : 'bg-white text-deep-brown hover:bg-primary-gold hover:text-white'
            }`}
          >
            Inquire
          </button>
          ...
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className={`lg:hidden transition-colors duration-300 ${scrolled || !isHome ? 'text-brand-primary' : 'text-white'}`}>
      ...
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 top-24 z-[40] bg-white animate-fade-in overflow-y-auto lg:hidden">
          <div className="p-8 space-y-10">
            {categoryGroups.map(item => (
              <div key={item.label}>
                <h3 className="text-[10px] uppercase tracking-[0.3em] font-bold text-primary-gold mb-6">{item.label}</h3>
